...
public class ServletEliminar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            clienteX.setId(Integer.parseInt(request.getParameter("txtId1")));
            
            boolean respuesta = clienteXDAO.EliminarCliente(clienteX);
            if (respuesta==true){
            out.println("Cliente eliminado exitosamente.");
        }else{
            out.println("Error al eliminar los datos del cliente.");
        }
        }
    }
